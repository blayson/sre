...
class FeaturesRepository(BaseRepository):
    async def get_all_feature_names_by_lang(
        ...
        review_stmt = (
            select([products.c.product_categories_id])
            .select_from(
        ...
        feature_stmt = (
            select([feature_names.c.text, feature_names.c.feature_names_id])
            .select_from(
                feature_names.join(
            ...
            .where(
                and_(
                    feature_names.c.product_categories_id == product_category_id,
                    feature_names.c.text.ilike(f"%{query}%"),
                )
            )
        )
